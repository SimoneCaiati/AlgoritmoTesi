...
class AccPath:
    ...
    def instantiate_ahrs_algorithm(self):
        self.offset = imufusion.Offset(self.sample_rate) #correzione di eventuali bias
    ...
    def process_sensor_data(self):
        ...
        for index in range(len(self.timestamp)):
            #self.gyroscope[index] = self.gyroscope[index]* (180.0 / math.pi)            #conversione dei radianti in gradi
            self.gyroscope[index] = self.offset.update(self.gyroscope[index])
            self.accelerometer[index]= self.accelerometer[index]             #da togliere per short_walk
    ...
    def identify_moving_periods(self):
        ...
        magnitudes = np.sqrt(np.sum(self.acceleration**2, axis=1))  #accelerazione risultante
        #print(magnitudes[0:800])
        print(f"massimo: {np.max(magnitudes)} media: {np.mean(magnitudes)} minimo: {np.min(magnitudes)}")
        for index in range(len(self.timestamp)):
            self.is_moving[index] = np.sqrt(
                self.acceleration[index].dot(self.acceleration[index])) >= np.mean(magnitudes)  # threshold = 3 m/s/s per short_walk
    ...
    def percorso(self):
        ...
        anim.save(self.path + "Animazione.gif", writer=animation.PillowWriter(fps))
