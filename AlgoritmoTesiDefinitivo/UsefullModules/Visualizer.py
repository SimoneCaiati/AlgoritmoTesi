...
class Visualizer:
    ...
    def plot_path(self, title="", n_sections=10, c1="green", c2="blue"):
            ...
            ax.set_zlabel("m")
            plt.legend()
            plt.savefig(self.mediaDir + "/" + "path_plot.png")
            plt.show() 
            plt.close()
    ...
    def animate_path(self, length_sec=15, fps=4, c1="green", c2="blue"):
        ...
        ax.set_zlabel("m")
        plt.legend()
        anim.save(self.mediaDir + "/" +"animated_path.gif", writer=animation.PillowWriter(fps))
        plt.show()
        plt.close()
        
    def plot_acceleration_data(self,nome_acc, Acc):
        ...
        ax.set_title(nome_acc)
        ax.set_ylabel("Acceleration (m/s^2)")
        ax.set_xlabel("Time (s)")
        ax.grid()
        ax.legend()
        plt.tight_layout()
        plt.savefig(self.mediaDir + "/" + nome_acc + ".png")
        plt.show()
        plt.close()
    
    def plot_euler_angles(self,nome_orient, Orient):
        ...
        plt.title(nome_orient)
        plt.xlabel("t [s]")
        plt.ylabel("rad/s")
        plt.grid()
        plt.legend()
        plt.savefig(self.mediaDir + "/" + nome_orient +".png")
        plt.show()
        plt.close()
